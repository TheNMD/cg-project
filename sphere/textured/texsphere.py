...
class TexSphere(object):
    def __init__(self, vert_shader, frag_shader, flag):
        self.flag = flag
        if self.flag == 0:
            self.vertices, self.indices, self.colors, self.normals, self.texcoords = sphere([0.0, 0.0, 0.0], 1, 50, 50) # center, radius, stacks, sectors - Sphere with stacks and sectors
        else:
            self.vertices, self.indices, self.colors, self.normals, self.texcoords = sphere1(5) # subdivision - Sphere from tetrahedron
